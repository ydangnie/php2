...
<head>
    ...
    <style>
        ...
        .variant-badge { font-size: 0.75rem; background: #f1f1f1; color: #333; border: 1px solid #ddd; padding: 2px 6px; border-radius: 4px; display: inline-block; margin-right: 3px; margin-bottom: 3px; }
    </style>
</head>
                    ...
                    <form action="/products/index" method="GET" class="row g-2 mb-4">
                        <div class="col-auto">
                            <div class="input-group">
                                <input type="text" name="tukhoa" class="form-control rounded-0" placeholder="Nhập tên sản phẩm..." value="<?= $tukhoa ?? '' ?>">
                        ...
                        <table class="table table-hover align-middle border">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th>Tên sản phẩm</th>
                                    <th>Phân loại (Size / Màu)</th> <th>Giá bán</th>
                                    <th>Danh mục</th>
                                ...
                                <?php else: ?>
                                    <?php foreach ($products as $product): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <small class="d-block text-primary"><?= $product['tendanhmuc'] ?? '-' ?></small>
                                                <small class="text-muted"><?= $product['tenthuonghieu'] ?? '-' ?></small>
                                            </td>
                                            <td class="text-center">
                                                <span class="fw-bold"><?= $product['soluong'] ?></span>
                                            </td>
                                            <td class="text-center">
                                                <a href="/products/edit/<?= $product['id'] ?>" class="btn btn-sm btn-outline-dark rounded-0 me-1" title="Sửa">
                    ...
                    <div class="d-flex justify-content-end mt-3">
                        <nav>
                            <ul class="pagination pagination-sm">
                                ...
                                <?php if($page > 1): ?>
                                    <li class="page-item"><a class="page-link text-dark" href="/products/index?page=<?= $page - 1 ?><?= $searchParams ?>">Trước</a></li>
                                <?php endif; ?>

                                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                    <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                        <a class="page-link <?= ($i == $page) ? 'bg-dark border-dark text-white' : 'text-dark' ?>" href="/products/index?page=<?= $i ?><?= $searchParams ?>"><?= $i ?></a>
                                    </li>
                                <?php endfor; ?>

                                <?php if($page < $totalPages): ?>
                                    <li class="page-item"><a class="page-link text-dark" href="/products/index?page=<?= $page + 1 ?><?= $searchParams ?>">Sau</a></li>
                                <?php endif; ?>
                            </ul>
                        </nav>
                    </div>
