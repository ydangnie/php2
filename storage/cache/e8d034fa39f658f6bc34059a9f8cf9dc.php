<!DOCTYPE html>
<html lang="vi">
<head>
    <meta charset="UTF-8">
    <title>Quản lý Đơn Hàng - Admin</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
</head>
<body class="bg-light">
    <div class="container-fluid p-0">
        <div class="d-flex min-vh-100">
            
            <?php echo $__env->make('layout.admin_sidebar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

            <div class="mx-auto" style="max-width: 1300px;">
                <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-4">
                    <h2 class="fw-bold text-uppercase mb-0">Quản lý Đơn hàng</h2>
                </div>

                <div class="card border-dark rounded-0 shadow-sm">
                    <div class="card-body p-0 table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="table-dark">
                                <tr>
                                    <th class="ps-3 py-3">Mã ĐH</th>
                                    <th>Khách hàng</th>
                                    <th>Ngày đặt</th>
                                    <th>Tổng tiền</th>
                                    <th>Thanh toán</th>
                                    <th>Trạng thái</th>
                                    <th class="text-center">Hành động</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php $__currentLoopData = $orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $od): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    <td class="ps-3 fw-bold">#DH<?php echo e($od['id']); ?></td>
                                    <td>
                                        <span class="fw-bold"><?php echo e($od['ten_nguoi_nhan']); ?></span><br>
                                        <small class="text-muted"><?php echo e($od['sdt']); ?></small>
                                    </td>
                                    <td><?php echo e(date('d/m/Y H:i', strtotime($od['created_at']))); ?></td>
                                    <td class="text-danger fw-bold"><?php echo e(number_format($od['tong_tien'])); ?>đ</td>
                                    <td>
                                        <?php if($od['trang_thai_tt'] == 'da_thanh_toan'): ?> 
                                            <span class="badge bg-success rounded-0">Đã TT</span>
                                        <?php else: ?> 
                                            <span class="badge border border-dark text-dark rounded-0">Chưa TT</span> 
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if($od['trang_thai_don'] == 'cho_xac_nhan'): ?> <span class="badge bg-secondary rounded-0">Chờ xác nhận</span>
                                        <?php elseif($od['trang_thai_don'] == 'dang_giao'): ?> <span class="badge bg-primary rounded-0">Đang giao</span>
                                        <?php elseif($od['trang_thai_don'] == 'hoan_thanh'): ?> <span class="badge bg-success rounded-0">Hoàn thành</span>
                                        <?php else: ?> <span class="badge bg-danger rounded-0">Đã hủy</span> <?php endif; ?>
                                    </td>
                                    <td class="text-center">
                                        <a href="/admin/chitietDonHang/<?php echo e($od['id']); ?>" class="btn btn-sm btn-dark rounded-0">Xem / Xử lý</a>
                                    </td>
                                </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
            
        </div>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html><?php /**PATH C:\xampp\htdocs\all_php\php11\app\views/admin/donhang/index.blade.php ENDPATH**/ ?>